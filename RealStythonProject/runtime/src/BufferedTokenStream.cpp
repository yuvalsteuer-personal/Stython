...
void BufferedTokenStream::lazyInit() {
  if (_needSetup) {
    std::cout<< "Line: 182, lazyInit();" <<std::endl;
    setup();
    std::cout<< "Line: 184, lazyInit();" <<std::endl;
  }
}

void BufferedTokenStream::setup() {
  _needSetup = false;
  std::cout<< "Line: 190, setup();" <<std::endl;
  sync(0);  std::cout<< "Line: 191, setup();sync(0);" <<std::endl;
...
void BufferedTokenStream::fill()
 {
  std::cout << "FILL, LINE 401" << std::endl;
  lazyInit();   std::cout << "FILL, LINE 402" << std::endl;
  const size_t blockSize = 1000;  std::cout << "FILL, LINE 403" << std::endl;
  while (true) {  std::cout << "FILL, LINE 404" << std::endl;
    size_t fetched = fetch(blockSize);  std::cout << "FILL, LINE 405" << std::endl;
    if (fetched < blockSize) {  std::cout << "FILL, LINE 406" << std::endl;
      return;
    }
  }
}
